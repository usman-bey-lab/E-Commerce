  ...
  return (
    <div className="pd-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span
          key={star}
          className={`pd-star ${star <= (hovered || rating) ? "filled" : "empty"} ${!readonly ? "clickable" : ""}`}
...
const ProductDisplay = ({
  ...
  const [sizeError, setSizeError]       = useState('');  // ← add this
  ...
  // ── Size selection — clears error when size is picked ──
    ...
    <div className="productdisplay">
      ...
      <div className="productdisplay-right">
        ...
        {userRating > 0 && !submitted && (
          <div className="pd-user-rating">
            <button className="pd-rate-btn" onClick={handleSubmitRating}>
              Submit Rating
            </button>
            <button
              className="pd-rate-cancel"
              onClick={() => { if (onRate) onRate(0); }}
            >
              Cancel
            </button>
          </div>
        ...
        {submitted && (
          <p className="pd-rating-success">✓ Thanks for your rating!</p>
        )}
        ...
        <div className="pd-size-section">
          ...
          <div className="pd-sizes">
            {SIZES.map((size) => (
              <button
                key={size}
                className={`pd-size-btn${selectedSize === size ? " active" : ""}${sizeError && !selectedSize ? " pd-size-btn--error" : ""}`}
          ...
          {/* ← error shows here, below the size buttons */}
          {sizeError && (
            <p className="pd-size-error">{sizeError}</p>
          )}
        </div>
